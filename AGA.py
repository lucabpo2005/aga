...
# RESTRICCIÓN 1 INTERACTIVA (Antena Grande, Mediana y Chica)
st.sidebar.subheader("📋 Restricción Nº 1")
r1_x = st.sidebar.number_input("Coeficiente Antena Grande en R1", value=1.0, key="r1x")
r1_y = st.sidebar.number_input("Coeficiente Antena Mediana en R1", value=1.0, key="r1y")
r1_z = st.sidebar.number_input("Coeficiente Antena Chica en R1", value=1.0, key="r1z")
lim_r1 = st.sidebar.number_input("Límite máximo R1 (≤)", min_value=1.0, value=15.0, step=1.0, key="r1lim")

# RESTRICCIÓN 2 INTERACTIVA (Mantiene coeficientes base del problema)
st.sidebar.subheader("📋 Restricción Nº 2")
r2_x = st.sidebar.number_input("Coeficiente x en R2", value=20.0, key="r2x")
r2_y = st.sidebar.number_input("Coeficiente y en R2", value=10.0, key="r2y")
r2_z = st.sidebar.number_input("Coeficiente z en R2", value=5.0, key="r2z")
lim_r2 = st.sidebar.number_input("Límite máximo R2 (≤)", min_value=1.0, value=200.0, step=10.0, key="r2lim")

# RESTRICCIÓN 3 INTERACTIVA (Antena Grande, Mediana y Chica)
st.sidebar.subheader("📋 Restricción Nº 3")
r3_x = st.sidebar.number_input("Coeficiente Antena Grande en R3", value=500.0, key="r3x")
r3_y = st.sidebar.number_input("Coeficiente Antena Mediana en R3", value=300.0, key="r3y")
r3_z = st.sidebar.number_input("Coeficiente Antena Chica en R3", value=200.0, key="r3z")
lim_r3 = st.sidebar.number_input("Límite máximo R3 (≤)", min_value=1.0, value=5000.0, step=100.0, key="r3lim")
...
# Matriz A estructurada horizontalmente de forma segura
...
if res.success:
    ...
    datos_tabla = {
        "Restricción Analizada": ["Restricción Nº 1 (Específica)", "Restricción Nº 2 (Base)", "Restricción Nº 3 (Específica)"],
        "Descripción en Pantalla": [
            f"Límite de Antenas: {r1_x}(Grande) + {r1_y}(Mediana) + {r1_z}(Chica) ≤ {lim_r1}",
            f"Capacidad General: {r2_x}x + {r2_y}y + {r2_z}z ≤ {lim_r2}",
            f"Presupuesto Antenas: {r3_x}(Grande) + {r3_y}(Mediana) + {r3_z}(Chica) ≤ {lim_r3}"
        ],
